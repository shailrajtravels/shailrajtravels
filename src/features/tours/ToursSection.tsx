...
import { tours as coreTours } from '../../data/tours';
import useEmblaCarousel from 'embla-carousel-react';
import Autoplay from 'embla-carousel-autoplay';
import { ChevronLeft, ChevronRight } from 'lucide-react';

export function ToursSection({ lang, t, packages }: { lang: 'mr' | 'en', t: any, packages?: any[] }) {
  const [selectedTour, setSelectedTour] = useState<TourData | null>(null);
  
  const mappedCoreTours: TourData[] = coreTours.map((t) => ({
    id: t.slug,
    slug: t.slug,
    image: t.heroContent.image,
    durationBadge: "Guided Tour",
    subtitle: t.title,
    title: t.title,
    location: "Pune",
    schedule: "Regular Departures",
    frequency: "Weekly",
    route: t.destinations.slice(0, 4),
    tags: t.highlights.slice(0, 2),
    seatsAvailable: 15,
    seatsTotal: 20,
    price: `₹${t.packages[0]?.price}`,
    itinerary: [],
    includes: t.packages[0]?.inclusions || []
  }));

  // Use DB packages if available, otherwise fallback to the newly created SEO core tours
  const displayPackages = packages && packages.length > 0 ? packages : mappedCoreTours;
