...
export const ReviewCard: React.FC<ReviewCardProps> = ({ review, className = '' }) => {
  return (
    <div className={`bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col h-full ${className}`}>
      <div className="flex justify-between items-start mb-4">
        <div className="flex gap-1">
          {[...Array(5)].map((_, i) => (
            <Star key={i} className={`w-4 h-4 ${i < review.rating ? 'fill-yellow-400 text-yellow-400' : 'fill-slate-200 text-slate-200'}`} />
          ))}
        </div>
      ...
      <div className="border-t border-slate-100 pt-4 flex items-center gap-3">
        {review.photoUrl ? (
          <img src={review.photoUrl} alt={review.customerName} className="w-10 h-10 rounded-full object-cover" />
        ) : (
